...
_Generated by BO-db-documenter on 2026-04-29. Do not edit manually — changes will be overwritten._
...
>
> **Wave 4 note (2026-04-29):** _No schema changes._
>
> - **S001-F-002** patched the existing `ollama` service block in `docker-compose.yml`: removed the host port binding (`11434:11434`) so the service is reachable only on the internal Compose bridge network, and added a model-pull entrypoint that runs `ollama serve` then `ollama pull ${LITELLM_OLLAMA_MODEL:-gemma3:12b}` on first start. The `ollama-data` named volume (already declared by S001-NFR-003) continues to back `/root/.ollama` inside the container — it caches LLM model weights, not application data, so the persistence inventory below is unchanged. The service remains gated by `profiles: ["ollama"]`; default `docker compose up -d` still brings up only `backend` + `frontend`. Adds a "Local LLM development" section to `README.md`. `.env.example` unchanged (`ENABLE_OLLAMA_CONTAINER=false` was already present).
> - **S001-F-005** wired the `ENABLE_DOCUMENT_SEARCH` feature flag (constant already present in `backend/config.py` since S001-F-004) into `backend/api/idirs.py` (`idirs_search`, `idirs_rag`) and `backend/api/search.py` (`/api/search/health`). Both IDIRS endpoints now early-return `503 {"error": "feature_disabled", "detail": "..."}` before opening any outbound TCP connection when the flag is off. `GET /api/search/health` surfaces the flag as `"document_search": "enabled"|"disabled"`. The frontend `AIChatInterface.tsx` detects `error: "feature_disabled"` and renders a localized notice via the new i18n key `documentSearch.notImplemented` added to `src/i18n/locales/{de,en}.json`. None of this touches persistent state — IDIRS is an external proxy target, never a local data store, and the gating only short-circuits the outbound call.
...
**Feature flags (no on-disk persistence; runtime gates only — added by S001-F-004, wired by S001-F-005):**
- `ENABLE_ANONYMIZATION` — gates `handle_anonymization` and `check_service_health`. Default: `False`.
- `ENABLE_DOCUMENT_SEARCH` — gates `idirs_search` / `idirs_rag` (early 503 + `feature_disabled`); surfaces in `/api/search/health`. Default: `False`. Wired in Wave 4 (S001-F-005).
- `ENABLE_UPLOAD` — placeholder for S001-F-006. Default: `False`.
- `ENABLE_OLLAMA_CONTAINER` — documented toggle in `.env.example` (default `false`); informational — actual gating is via the Compose `--profile ollama` flag. Wave 4 (S001-F-002).
...
- `IDIRS_BASE_URL` — external service URL, default `http://localhost:8010`. Calls are short-circuited when `ENABLE_DOCUMENT_SEARCH=false` (S001-F-005).
...
- `LITELLM_OLLAMA_HOST` — URL of the Ollama instance the proxy forwards to. Default: `http://host.docker.internal:11434`. When using the Compose-managed Ollama (S001-F-002), the LiteLLM service reaches it via the internal hostname `http://ollama:11434` (no host port binding).
- `LITELLM_OLLAMA_MODEL` — documents the active model in `litellm/.env.example`. Default: `gemma3:12b`. Note: changing the model also requires editing `litellm/config.yaml` because LiteLLM does not env-substitute the `litellm_params.model` field. The Compose `ollama` service entrypoint also reads this var to drive `ollama pull` on first start (S001-F-002).
...
**Note (S001-F-004):** Upload endpoints will become gated behind the `ENABLE_UPLOAD` feature flag in a future wave (S001-F-006). Current upload code paths are unchanged in Wave 4.
...
- `src/i18n/locales/de.json` — German strings (source of truth after S001-F-008). Wave 2 added `context.offlineNotice`. Wave 3 added `anonymization.notImplemented`. Wave 4 added `documentSearch.notImplemented` (S001-F-005).
- `src/i18n/locales/en.json` — English strings. Wave 2 added `context.offlineNotice`. Wave 3 added `anonymization.notImplemented`. Wave 4 added `documentSearch.notImplemented` (S001-F-005).
...
12. Pass feature-flag env vars (`ENABLE_ANONYMIZATION`, `ENABLE_DOCUMENT_SEARCH`, `ENABLE_UPLOAD`) as runtime env vars; they default to `False`. They do not affect persistence — they gate request handlers. `ENABLE_DOCUMENT_SEARCH` is wired in Wave 4 (S001-F-005).
...
14. The Compose-managed `ollama` service (Wave 4 S001-F-002) is profile-gated under `--profile ollama`; default `docker compose up -d` does not start it. When started, its entrypoint pulls `${LITELLM_OLLAMA_MODEL:-gemma3:12b}` into the `ollama-data` volume on first run. The service has **no host port binding** — it is reachable only on the internal Compose bridge network at `http://ollama:11434`.
